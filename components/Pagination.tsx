      ...
      <ul className="flex items-center justify-center gap-2">
        <li>
          <button
            ...
            className="flex h-10 min-w-10 items-center justify-center rounded-lg border border-gray-300 bg-white px-2 text-base font-medium text-gray-900 hover:bg-gray-100 transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <span>
              <svg
                width="20"
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
          <li key={page}>
            <button
              ...
              className={`flex h-10 min-w-10 items-center justify-center rounded-lg border px-2 text-base font-medium transition ${
                currentPage === page
                  ? 'border-[#3373FF] bg-[#3373FF] text-white'
