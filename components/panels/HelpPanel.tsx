...
export default function HelpPanel() {
  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-gray-100 text-gray-900 placeholder-gray-400 rounded-lg pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#3373FF] focus:bg-white"
          />
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 gap-3">
          {helpCategories.map((category, index) => (
            <div
              key={index}
              className="flex items-start gap-4 p-4 border border-gray-200 rounded-lg hover:border-[#3373FF] hover:bg-[rgba(154, 121, 255, 0.1)] transition cursor-pointer"
            >
              <div className="text-[#3373FF] mt-0.5">{category.icon}</div>
      ...
      {/* Contact Support */}
      <div className="bg-gradient-to-r from-[#3373FF] to-[#265ACC] rounded-lg p-6 text-white">
        <h3 className="font-semibold mb-2">Still need help?</h3>
        <p className="text-sm opacity-90 mb-4">Our support team is available 24/7 to assist you.</p>
        <button className="bg-white text-[#3373FF] px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100 transition">
          Contact Support
        </button>
      </div>
    </div>
